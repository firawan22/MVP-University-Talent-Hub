...
let RecommendationsService = class RecommendationsService {
    studentRepo;
    oppRepo;
    constructor(studentRepo, oppRepo) {
        this.studentRepo = studentRepo;
        this.oppRepo = oppRepo;
    }
    async recommendOpportunities(studentId) {
    ...
    async recommendSkills(studentId) {
        const student = await this.studentRepo.findOne({ where: { id: studentId } });
        if (!student || !student.skills || student.skills.length === 0) {
            return [];
        }
        ...
        for (const peer of allStudents) {
            if (peer.id === studentId || !peer.skills)
                continue;
...
exports.RecommendationsService = RecommendationsService = __decorate([
    ...
    __param(1, (0, typeorm_1.InjectRepository)(opportunity_entity_1.OpportunityEntity)),
